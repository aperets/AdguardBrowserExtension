/**
 * Build extension with the AdGuard API, which can be included to another extension.
 * 1. Copying assistant scripts
 * 2. Copying extension directory
 * 3. Concat scripts from `document_start` and `document_end` params getting from manifest.json
 * 4. Concat all scripts from `API_SCRIPTS` files and save in 'youtube-blocker-extension.js'
...
const paths = {
    sample: path.join('Extension/api/youtube-blocker-extension/**/*'),
    ...
    dest: path.join(BUILD_DIR, BRANCH, 'youtube-blocker-extension'),
};
...
const createArchive = (done) => {
    ...
    if (BRANCH === BRANCH_DEV) {
        return gulp.src(dest.inner)
            .pipe(zip('youtube-blocker-extension.zip'))
            .pipe(gulp.dest(dest.buildDir));
    }

    return gulp.src(dest.inner)
        .pipe(zip(`youtube-blocker-extension-${BRANCH}.zip`))
        .pipe(gulp.dest(dest.buildDir))
        // youtube-blocker-extension.zip artifact
        .pipe(rename('youtube-blocker-extension.zip'))
